...
def load_history():
    ...
    try:
        # load urls
        page_visited = int(pickle.load(open("./国防科技信息网_data/page_visited.p", "rb")))  # 记录该页面是否被访问过
    except:
        page_visited = 1
    return page_visited
...
def read_article(soup):
    ...
    title = title.replace('-', '_').replace('\r', '').replace('*', '').replace(' ', '_').replace('>', '').replace('?', '').replace('#', '').replace(';', '').replace('_', '').replace('\'', '').replace('\"', '').replace(
        '/', ' ').replace('\n', '').strip()
    filename = './国防科技信息网_data/' + title + '.txt'
...
page = "http://www.dsti.net/Information/NewsList/"  # 记录需要访问的页面链接
article_path = 'http://www.dsti.net'

page_visited = load_history()  # 加载访问历史

TOTAL_PAGE = 6318  # 该网站可访问的总页面数
page_num = page_visited + 2  # 开始访问的页面
print(page_num)
...
while page_num <= TOTAL_PAGE:  # 需要访问的页面还没有访问完
    ...
    pickle.dump(page_num, open("./国防科技信息网_data/page_visited.p", "wb"))
