...
def load_history():
    ...
    try:
        # load urls
        page_visited = int(pickle.load(open("./中国军网_我军装备_data/page_visited.p", "rb")))  # 记录该页面是否被访问过
    except:
        page_visited = 1
    return page_visited
...
def read_article(soup):
    ...
    try:
        ...
        title = title.replace('-', '_').replace('|', '').replace(' ', '_').replace('>', '').replace('?', '').replace('#', '').replace(
            ...
            '/', ' ').replace('\n', '').strip()
        filename = './中国军网_我军装备_data/' + title + '.txt'
...
page = "http://www.81.cn/bqtd/node_105013"  # 记录需要访问的页面链接
article_path = 'http://www.81.cn/bqtd/'

page_visited = load_history()  # 加载访问历史

TOTAL_PAGE = 2  # 该网站可访问的总页面数
...
while page_num <= TOTAL_PAGE:  # 需要访问的页面还没有访问完
    ...
    pickle.dump(page_num, open("./中国军网_我军装备_data/page_visited.p", "wb"))
