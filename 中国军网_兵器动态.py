...
def load_history():
    ...
    try:
        # load urls
        page_visited = int(pickle.load(open("./中国军网_兵器动态_data/page_visited.p", "rb")))  # 记录该页面是否被访问过
    except:
        page_visited = 1
    return page_visited
...
def read_article(soup):
    ...
    try:
        ...
        # 将获取的文章保存到txt文件中
        title = title.replace('-', '_').replace('|', '').replace(' ', '_').replace('>', '').replace('?', '').replace('#', '').replace(';', '').replace('_', '').replace('\'', '').replace('\"', '').replace(
            '/', ' ').replace('\n', '').strip()
        filename = './中国军网_兵器动态_data/' + title + '.txt'
...
page = "http://www.81.cn/bqtd/node_61571"  # 记录需要访问的页面链接
...
while page_num <= TOTAL_PAGE:  # 需要访问的页面还没有访问完
    ...
    pickle.dump(page_num, open("./中国军网_兵器动态_data/page_visited.p", "wb"))
