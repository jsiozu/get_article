...
def load_history():
    ...
    try:
        # load urls
        page_visited = int(pickle.load(open("./" + dir_name + "/page_visited.p", "rb")))  # 记录该页面是否被访问过
    except:
        page_visited = 1
    return page_visited
...
page = "http://www.81.cn/bqtd/node_105014"  # 记录需要访问的页面链接
article_path = 'http://www.81.cn/bqtd/'

page_visited = load_history()  # 加载访问历史

dir_name = '中国军网_兵器专栏_data'

TOTAL_PAGE = 1  # 该网站可访问的总页面数
